...
import { evaluate_expr, Scope, type EvalContext } from '../index';

function create_ctx(state: any): EvalContext {
...
describe('evaluateExpr runtime', () => {
  ...
  it('evaluates quantifiers with local scopes', () => {
    ...
    expect(evaluate_expr(exists, ctx)).toBe(true);
    ...
    expect(evaluate_expr(forall, ctx)).toBe(true);
    ...
    expect(evaluate_expr(forallFail, ctx)).toBe(false);

    // ensure outer binding remains intact
    expect(ctx.scope.get_var('$n')).toEqual([1, 2, 3]);
  });
});
